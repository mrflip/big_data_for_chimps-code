    ...
    class TutumNode < ::Tutum::NodeCluster
      ...
      def start_using_manifest(mft)
        if deployable?
          ...
          update(target_num_nodes: mft.instances, tags: tutum_tags_list(mft))
        end
      end

      def stop_using_manifest(mft)
        update(target_num_nodes: 0, tags: tutum_tags_list(mft))
      end
      ...
      def tutum_tags_list(mft)
        (mft.container_names + ["region--#{mft.region}"]).map{|tag| {name: tag} }
      end
