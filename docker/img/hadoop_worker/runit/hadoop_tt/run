...
echo "********************************"
echo 
echo "Tasktracker runit script invoked at `date`"
echo
...
echo ; echo "Launching Tasktracker" ; echo

cd /var/lib/hadoop-0.20-mapreduce
exec chpst -u $daemon_user /usr/bin/hadoop --config $HADOOP_CONF_DIR tasktracker < /dev/null
