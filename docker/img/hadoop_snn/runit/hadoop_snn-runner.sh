...
echo "********************************"
echo 
echo "Secondarynamenode runit script invoked at `date`"
echo
...
if [ -f "/etc/default/hadoop-hdfs-secondarynamenode" ] ; then
...
echo ; echo "Launching Secondarynamenode" ; echo

cd /var/lib/hadoop-hdfs
exec chpst -u hdfs /usr/bin/hdfs --config $HADOOP_CONF_DIR secondarynamenode < /dev/null
